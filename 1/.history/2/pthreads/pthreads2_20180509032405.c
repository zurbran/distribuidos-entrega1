...
typedef struct mulCuadradaTriangularSup_args
{
	int start;
	int end;
	double *A;
...
{
    average *argsPointer = (average *) args;
    double total = 0.0;

    for (int i = argsPointer->start; i <= argsPointer->end; i++)
    {
        total += argsPointer->matrix[i];
    }

    total = total/(argsPointer->end - argsPointer->start + 1);
...
void *triangularInferiorPorCuadrada_thread(void *args)
{
	...
	for(int i = arg->start; i <= arg->end; i++)
	{
		...
		{
			setValor(arg->C, i, j, arg->ordenC, arg->N, 0);
			for(int k = 0; k < i + 1; k++)
			{
				setValor(arg->C, i, j, arg->ordenC, arg->N, getValor(arg->C, i, j, arg->ordenC, arg->N) + getValor(arg->L, i, k, arg->ordenL, arg->N)*getValor(arg->B, k, j, arg->ordenB, arg->N));
			}
		}
	}  
}
...
void *cuadradaPorTriangularSuperior_thread(void *args)
{
	mulCuadradaTriangularSup_args *arg = (mulCuadradaTriangularSup_args *) args;
	...
	for(int i = arg->start; i <= arg->end; i++)
	{
		for(int j = 0; j < arg->N; j++)
		{
			...
			for(int k = 0; k < j + 1; k++)
			{
				setValor(arg->C, i, j, arg->ordenC, arg->N, getValor(arg->C, i, j, arg->ordenC, arg->N) + getValor(arg->A, i, k, arg->ordenA, arg->N)*getValor(arg->U, k, j, arg->ordenU, arg->N));
			}
		}
	}
}
...
{
	pthread_t threads[T];
	mulCuadradaTriangularSup_args args[T];
	int extra = N % T;
	for(int i = 0; i < T; i++)
	{
		// Para corregir cuando N no es divisible por T
